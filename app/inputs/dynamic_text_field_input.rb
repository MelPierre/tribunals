class DynamicTextFieldInput < SimpleForm::Inputs::Base
  ...
  def input
    template.content_tag(:span, class: "input string optional #{attribute_name}", style: "margin-left:45px") do
  ...
  private
    ...
    def input_html_options(position)
      ...
      {id: "#{model}_#{position}", name: "#{name}", style:'width:390px;margin:5px;'}
    end
